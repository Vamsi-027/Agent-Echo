...
@click.option("--schema-path", type=click.Path(exists=True), help="Path to schema.sql file")
...
def seed_fake_events():
    ...
    sample_events = [
        # Day -4 (4 days ago)
        {
            ...
            "detail": {"duration_minutes": 60, "attendees_count": 5, "organizer": "Vamsi"}
        },
        {
            "days_ago": 4,
            "source": "git",
            ...
            "detail": {"repo": "agent-echo", "commit_hash": "a1b2c3d", "files_changed": ["db/schema.sql", "db/db.py"]}
        },
        {
            "days_ago": 4,
            "source": "note",
        ...
        # Day -3
        {
            ...
            "detail": {"duration_minutes": 45, "attendees_count": 12, "organizer": "Product Team"}
        },
        {
            "days_ago": 3,
            "source": "file",
            ...
            "detail": {"project": "agent-echo", "lines_added": 120, "lines_removed": 15}
        },
        {
            "days_ago": 3,
            "source": "note",
            ...
            "detail": {"category": "dev_log", "notes": "Got 100% valid JSON responses from Claude. Very reliable."}
        },
        # Day -2
        {
            "days_ago": 2,
        ...
        },
        {
            ...
            "detail": {"repo": "agent-echo", "commit_hash": "c4d5e6f", "files_changed": ["notification/router.py", "notification/telegram_channel.py"]}
        },
        {
            "days_ago": 2,
            "source": "note",
        ...
        },
        {
            ...
            "detail": {"repo": "agent-echo", "commit_hash": "f7g8h9i", "files_changed": ["publisher/linkedin_client.py"]}
        },
        {
            "days_ago": 1,
            "source": "note",
        ...
        },
        {
            ...
            "detail": {"repo": "agent-echo", "commit_hash": "t1u2v3w", "files_changed": ["tests/test_content_queue.py"]}
        },
        {
            "days_ago": 0,
            "source": "note",
    ...
    for ev in sample_events:
        ...
        event_time = event_date.replace(hour=ev["time_offset_hours"], minute=0, second=0, microsecond=0)
...
def digest_cmd(date):
    ...
    try:
        ...
        if result.get("status") == "success" and result.get("digest"):
            ...
            click.echo(f"Daily digest generated (ID: {digest_row['id']}, Date: {digest_row['date']}, Pillar: {digest_row['suggested_pillar']})")
            click.echo(f"Highlights: {digest_row['highlights_json']}")
        else:
            click.echo(f"Digest skipped or failed. Status: {result.get('status')} | Error: {result.get('error') or 'No activity events found'}")
    except Exception as e:
...
def generate_cmd(date):
    ...
    try:
        ...
        if drafts:
            ...
            for d in drafts:
                click.echo(f"  - Draft ID: {d['id']} | Pillar: {d['pillar']} | Format: {d['format_type']}")
        else:
...
@click.option("--edit", nargs=2, type=(int, str), help="Provide custom instruction to edit a draft (args: draft_id, instruction)")
def review_cmd(approve, skip, edit):
    ...
    if approve:
        ...
        try:
            cursor.execute("UPDATE drafts SET status='rejected', updated_at=datetime('now') WHERE id=?", (skip,))
            conn.commit()
        ...
        try:
            ...
            click.echo(f"New draft variant generated (ID: {new_draft['id']}). Review it via review command.")
        except Exception as e:
            click.echo(f"Error editing draft: {e}", err=True)
            
    else:
        # List all pending drafts
        cursor.execute("SELECT id, pillar, format_type, text_content, created_at FROM drafts WHERE status='pending_review'")
...
@click.option("--dry-run", is_flag=True, default=None, help="Force dry run (bypasses .env setting)")
...
def authorize_cmd():
    ...
    click.echo(f"\n   {auth_url}\n")
    click.echo("2. After authorizing, you will be redirected to localhost (which will fail to load or show a blank page).")
    click.echo("3. Copy the entire redirect URL from your browser's address bar or just the 'code' query parameter.")
    ...
    if "code=" in redirect_input:
        ...
        try:
            ...
            if "code" in params:
                ...
                click.echo("Error: Could not extract authorization 'code' from the URL.", err=True)
                return
        except Exception as e:
    ...
    try:
        ...
        if refresh_token:
            click.echo(f"Refresh token saved. Expires in {refresh_token_expires_in // 86400} days.")
        else:
            click.echo("Warning: No refresh token returned. Long-term automatic updates may require re-auth.")
...
def run_pipeline_cmd(date):
    ...
    try:
        if date:
            ...
            run_daily_pipeline()
            click.echo("Pipeline completed. Drafts dispatched to configured notification channels.")
    except Exception as e:
...
def telegram_bot_cmd():
    ...
    try:
        from notification.telegram_channel import run_telegram_bot_foreground
