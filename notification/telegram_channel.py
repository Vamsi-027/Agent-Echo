...
def get_draft_message(draft_id: int) -> str:
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT pillar, format_type, text_content, hashtags, scheduled_time FROM drafts WHERE id=?", (draft_id,))
    row = cursor.fetchone()
    conn.close()
    if not row:
        return ""
...
class TelegramChannel(NotificationChannel):
    ...
    def send(self, message: str, actions: list[str] | None = None) -> bool:
        ...
        if not allowed_users:
            logger.warning("No allowed Telegram user IDs configured in TELEGRAM_ALLOWED_USER_IDS.")
            return False
            
        success = False
        url = f"https://api.telegram.org/bot{self.bot_token}/sendMessage"
        
        # Format inline keyboard if actions are passed
        reply_markup = None
        if actions:
            row1 = []
            row2 = []
            for act in actions:
                if act.startswith("approve_"):
                    ...
                    row1.append({"text": "📅 Reschedule", "callback_data": f"reschedule_{draft_id}"})
        ...
        for chat_id in allowed_users:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    logger.warning(f"Telegram returned error code {resp.status_code}: {resp.text}")
            except Exception as e:
                logger.error(f"Failed to post notification to Telegram chat {chat_id}: {e}")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_user_allowed(user_id):
        ...
        await update.message.reply_text("Unauthorized. Your user ID is not whitelisted.")
        return
...
async def start_telegram_weekly_review(chat_id: int, update: Update):
    ...
    if not rows:
        await update.message.reply_text("✅ All published posts already have performance metrics recorded.")
        return
        
    weekly_review_state[chat_id] = {
        "posts": rows,
        "current_index": 0
    }
    
    await update.message.reply_text(f"📊 Found {len(rows)} published post(s) missing performance logs. Let's log them now:")
...
async def prompt_next_weekly_post(chat_id: int, update: Update):
    ...
    snippet = post["text_content"][:100] + "..." if len(post["text_content"]) > 100 else post["text_content"]
    prompt_msg = (
    ...
    if update.message:
        ...
        await update.callback_query.message.reply_text(prompt_msg, parse_mode="Markdown")
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("approve_"):
        ...
        try:
            ...
            if not base_msg:
                base_msg = escape_markdown(query.message.text)
...
async def handle_user_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in pending_edits:
        ...
        try:
            ...
            actions = [f"approve_{new_draft['id']}", f"edit_{new_draft['id']}", f"skip_{new_draft['id']}"]
            
            # Use TelegramChannel helper to construct buttons
            tg = TelegramChannel()
            tg.send(msg, actions)
        except Exception as e:
    ...
    # 2. Handle reschedule instructions
        ...
        try:
            ...
            if not parsed_dt:
                raise ValueError("Format must be YYYY-MM-DD HH:MM (e.g. 2026-06-16 14:30)")
            ...
            def db_reschedule():
                ...
                cursor.execute("SELECT id FROM content_queue WHERE draft_id = ?", (draft_id,))
    ...
    # 3. Handle weekly performance review stats
        ...
        parts = text.split(",")
        if len(parts) != 4:
            await update.message.reply_text("⚠️ Invalid format. Please reply in format: `impressions, reactions, comments, reposts` (e.g. `2000, 40, 5, 2`).")
            return
            
        try:
            metrics = [int(p.strip()) for p in parts]
        except ValueError:
            await update.message.reply_text("⚠️ Metrics must be integers. Try again in format: `impressions, reactions, comments, reposts`.")
            return
        ...
        def save_metrics():
            ...
            cursor.execute(
                ...
                (post["linkedin_post_urn"], metrics[0], metrics[1], metrics[2], metrics[3])
            )
            conn.commit()
            conn.close()
        ...
        if state["current_index"] < len(state["posts"]):
            ...
            await update.message.reply_text("🔄 All stats recorded. Initiating Claude optimization report...")
            ...
            await update.message.reply_text("📊 *Claude Performance & Reweighting Report*\n\n" + out)
...
def run_telegram_bot_daemon():
    ...
    if not token or token == "placeholder_bot_token":
        logger.warning("TELEGRAM_BOT_TOKEN is placeholder or not set. Bot listener will not be started.")
        return None
    ...
    t = threading.Thread(target=application.run_polling, kwargs={"stop_signals": None, "close_loop": False}, daemon=True)
    t.start()
    return t
