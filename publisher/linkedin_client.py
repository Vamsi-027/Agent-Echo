...
def post_text(author_urn: str, text: str) -> requests.Response:
    ...
    if is_dry_run:
        ...
        resp.headers = {"x-restli-id": "urn:li:share:mock_text_post_" + os.urandom(4).hex()}
...
def upload_carousel(author_urn: str, slide_image_paths: list[str]) -> str:
    ...
    is_dry_run = os.getenv("DRY_RUN", "true").lower() == "true"
    if is_dry_run:
        logger.info(f"[DRY RUN] Uploading carousel PDF generated from {slide_image_paths}")
...
def get_my_member_urn() -> str:
    ...
    if not token:
        raise ValueError("Access token not found in keyring. Please run authorization first.")
...
def post_draft_to_linkedin(draft: dict) -> requests.Response:
    ...
    if format_type == "text" or format_type == "long_form":
        ...
        media_refs = json.loads(draft["media_refs_json"]) if draft.get("media_refs_json") else []
        image_path = media_refs[0] if media_refs else None
        
        if is_dry_run:
            logger.info(f"[DRY RUN] Image post draft {draft['id']} containing image: {image_path}")
            resp = requests.Response()
            resp.status_code = 201
            resp.headers = {"x-restli-id": "urn:li:share:mock_image_post_" + os.urandom(4).hex()}
            resp._content = b'{"id": "urn:li:share:mock_image_post"}'
            return resp
            
        if not image_path:
            raise ValueError(f"Draft {draft['id']} of format 'image' has no media_refs.")
        ...
        if is_dry_run:
            logger.info(f"[DRY RUN] Carousel post draft {draft['id']} containing {len(media_refs)} slides")
            resp = requests.Response()
            resp.status_code = 201
            resp.headers = {"x-restli-id": "urn:li:share:mock_carousel_post_" + os.urandom(4).hex()}
            resp._content = b'{"id": "urn:li:share:mock_carousel_post"}'
            return resp
            
        if not media_refs:
            raise ValueError(f"Draft {draft['id']} of format 'carousel' has no media_refs.")
        ...
        if is_dry_run:
            logger.info(f"[DRY RUN] Video post draft {draft['id']} containing video: {video_path}")
            resp = requests.Response()
            resp.status_code = 201
            resp.headers = {"x-restli-id": "urn:li:share:mock_video_post_" + os.urandom(4).hex()}
            resp._content = b'{"id": "urn:li:share:mock_video_post"}'
            return resp
            
        if not video_path:
            raise ValueError(f"Draft {draft['id']} of format 'video' has no media_refs.")
        ...
        poll_data = json.loads(draft["media_refs_json"]) if draft.get("media_refs_json") else {}
        question = poll_data.get("question")
        options_list = poll_data.get("options", [])
        duration = poll_data.get("duration", "THREE_DAYS")
        
        if is_dry_run:
            logger.info(f"[DRY RUN] Poll post draft {draft['id']}: Q: '{question}' | Options: {options_list} | Duration: {duration}")
            resp = requests.Response()
            resp.status_code = 201
            resp.headers = {"x-restli-id": "urn:li:share:mock_poll_post_" + os.urandom(4).hex()}
            resp._content = b'{"id": "urn:li:share:mock_poll_post"}'
            return resp
            
        if not question or not options_list:
            raise ValueError(f"Draft {draft['id']} of format 'poll' has missing question or options in media_refs_json.")
...
def upload_video(author_urn: str, video_path: str) -> str:
    ...
    for attempt in range(12):
        time.sleep(5)
        status_url = f"https://api.linkedin.com/rest/videos/{video_urn}"
    ...
    raise TimeoutError("LinkedIn video processing timed out (still not AVAILABLE after 60s).")
